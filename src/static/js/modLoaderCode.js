    ...
    mods.forEach(async function(mod) {
        ...
        if(modsLoaded.length == allModsLength) {
            ...
            for(let i = 0; i < modsLoaded.length; i++) {
                ...
                if(modData.mod.dataset.awardimageurls && modData.mod.dataset.awardimageurls.split(", ").length > 1) {
                    cycleAwards(modView.querySelector(".mod-trophy"), modData.mod.dataset.awardimageurls.split(", "), 0);
                }
...
function cycleAwards(img, awardUrls, index) {
    ...
    setTimeout(() => cycleAwards(img, awardUrls, (index + 1) % awardUrls.length), 2000);
}

...
code1Picker.addEventListener("change", (e) => {
  ...
  reader.addEventListener(
    ...
    () => {
      try {
        let election = extractElectionDetails(
          reader.result,
          "uploaded mod",
        ).election_json;
        ...
        let runningMates = extractRunningMates(
          reader.result,
          "uploaded mod",
        ).running_mate_json;
        let tempElection = extractTempElection(reader.result, "uploaded mod")
          .temp_election_list[0];

        setUpCode2Submissions(election, candidates, runningMates, tempElection);
      } catch (e) {
        alert(
          "There was an error processing your code 1. Are you sure you submitted the right file?",
        );
...
function setUpCode2Submissions(
  election,
  candidates,
  runningMates,
  tempElection,
  ...
  let code2Names = modCandidates
    .filter((x) => x.fields.is_active)
    .map((x) =>
      runningMates
        .filter((y) => y.fields.candidate == x.pk)
        .map(
          (x) =>
            `${displayYear}_${getLastNameFromPk(candidates, x.fields.candidate)}${getLastNameFromPk(candidates, x.fields.running_mate)}.html`,
        ),
    );
  ...
  document.getElementById("submitArea").innerHTML +=
    `<button id="submitButton" onclick="submitMod()">Submit Mod</button><div>Please only press this button once</div><p style="color:red" id="warning"></p>`;
}
...
function checkAllPickers() {
  ...
  if (!allCode2s) {
    document.getElementById("warning").innerText =
      "Warning: Not all candidates/running mates have a code 2. If this is intentional because a candidate is not playable you are fine. Otherwise make sure all are uploaded before submitting.";
  } else {
    document.getElementById("warning").innerText = "";
  }
}
...
async function submitMod() {
  ...
  const tags = Array.from(document.getElementsByClassName("tagCheckbox"))
    .filter((x) => x.checked)
    .map((x) => x.name);
  ...
  document.getElementById("submitArea").innerHTML =
    "Submitting please wait...If there is an error refresh the page before trying again";

  let req = await fetch(
    "https://campaigntrailmojo.eu.pythonanywhere.com/update-repo",
    {
      method: "POST",
      body: data,
    },
  ...
  if (!req.ok) {
    alert(
      "There was an error submitting your mod request to the server! See the console for details.",
    );
    console.log("Request:", req);
  } else {
    alert("Mod submitted! Check CTS Github for status.");
    window.location.href =
      "https://github.com/campaign-trail-showcase/campaign-trail-showcase.github.io/pulls";
  }
}

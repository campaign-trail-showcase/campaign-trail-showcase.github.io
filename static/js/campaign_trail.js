...
function handleFinalResults(t) {
  ...
  e.final_overall_results.sort((a, b) => {
...
function nextQuestion() {
  ...
  if (e.primary) {
    ...
    */
    ...
    a = primaryFunction(
      window.primary_breaks.includes(e.question_number),
      window.primary_breaks,
    );
    if (a) {
      e.corQuestion = true;
      return false;
...
function getGameModeDescription(gameModeId) {
  ...
  $("#opponent_selection_description_window").html(gameModeDescription);
}
...
function renderOptions(electionId, candId, runId) {
  ...
  getGameModeDescription(gameTypeId.value);
...
function rFunc(t, i) {
  ...
  const cachedNationalPopularVoteEstimate = latestCandidates.map((c) => {
    const hasNoVotes = c.popvs === 0 && c.evvs === 0;
    return hasNoVotes ? '' : `<b>${c.fields.last_name}</b> - ${(c.pvp * 100).toFixed(1)}%<br>`
  }).join("");
  const cachedElectoralVoteEstimate = latestCandidates.reduce((acc, c, idx) => {
    if (evArray[idx] > 0) {
  ...
  const hoverHandler = (_evt, data) => {
    ...
    window.nn3 = evArray;
    nationalPopularVoteEstimate = cachedNationalPopularVoteEstimate;
    electoralVoteEstimate = cachedElectoralVoteEstimate;
    window.evestt = 0;
